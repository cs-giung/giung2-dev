...
def launch(config, print_fn):
    ...
    def apply_fn(images, params):
        f_vecs = model.get_image_features(images, params={'logit_scale': pretrained_params['logit_scale'], **params['ext']})
        f_vecs = f_vecs / jnp.sqrt(jnp.sum(f_vecs**2, axis=-1, keepdims=True))
    ...
    state = train_state.TrainState.create(
        ...
        tx       = optimizer,
        params   = {
            'ext': {'vision_model': pretrained_params['vision_model'],
                    'visual_projection': pretrained_params['visual_projection']},
    ...
    _state = jax.device_get(jax.tree_util.tree_map(lambda x: x[0], state))
    def predict(images):
        f_vecs = model.get_image_features(images, params={'logit_scale': pretrained_params['logit_scale'], **_state.params['ext']})
        f_vecs = f_vecs / jnp.sqrt(jnp.sum(f_vecs**2, axis=-1, keepdims=True))
