...
def launch(config, print_fn):
    ...
    # load pre-trained variables if specified
    if config.model_prior:
        pretrained_ckpt = checkpoints.restore_checkpoint(config.model_prior, target=None)
        pretrained_ckpt['params']['Dense_0'] = unfreeze(variables['params']['Dense_0'])
        if config.model_zero_head:
            pretrained_ckpt['params']['Dense_0'] = jax.tree_util.tree_map(
                jnp.zeros_like, pretrained_ckpt['params']['Dense_0'])
        variables = pretrained_ckpt
    ...
    def step_trn(state, batch, config, scheduler):
        ...
        def loss_fn(params):
            ...
            _, new_model_state = state.apply_fn({
                ...
                use_running_average = config.model_freeze_bn)
...
def main():
    ...
    parser.add_argument('--optim_ne', default=100, type=int,
