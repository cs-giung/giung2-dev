...
def launch(config, print_fn):
    ...
    def step_trn(state, batch, config, scheduler):
        ...
        scale = jax.tree_util.tree_map(
            lambda p: jnp.abs(p) if len(p.shape) == 4 else jnp.ones_like(p), state.params)
        grads = jax.tree_util.tree_map(
            lambda g: g / jnp.sqrt(sum([
                jnp.sum(jnp.square(s * p)) for p, s in zip(jax.tree_util.tree_leaves(grads), jax.tree_util.tree_leaves(scale))
            ])), grads)
        
        scale = jax.tree_util.tree_map(
            lambda p: config.rho * jnp.square(p) if len(p.shape) == 4 else config.rho * jnp.ones_like(p), state.params)
        noisy_params = jax.tree_util.tree_map(lambda p, g, s: p + s * g, state.params, grads, scale)
