...
from giung2.models.layers import ConvNormalRankOneBNN, DenseNormalRankOneBNN
...
def launch(config, print_fn):
    ...
    if config.model_style == 'BN-ReLU':
        model = _ResNet(
            conv = partial(ConvNormalRankOneBNN, use_bias=False, use_ensemble_bias=False,
                           kernel_init=jax.nn.initializers.he_normal(),
                           bias_init=jax.nn.initializers.zeros,
                           r_base=1.0, r_mean_init=jax.nn.initializers.zeros,
                           s_base=1.0, s_mean_init=jax.nn.initializers.zeros,
                           r_rawstd_init=jax.nn.initializers.constant(math.log(math.exp(config.stddev) - 1.0)),
                           s_rawstd_init=jax.nn.initializers.constant(math.log(math.exp(config.stddev) - 1.0)),
                           ensemble_size=config.ensemble_size),
            fc   = partial(DenseNormalRankOneBNN, use_bias=False, use_ensemble_bias=True,
                           kernel_init=jax.nn.initializers.he_normal(),
                           bias_init=jax.nn.initializers.zeros,
                           r_base=1.0, r_mean_init=jax.nn.initializers.zeros,
                           s_base=1.0, s_mean_init=jax.nn.initializers.zeros,
    ...
    def initialize_model(key, model):
        ...
        return init({'params': key}, jnp.ones(
            (config.ensemble_size,) + dataloaders['image_shape'][1:], model.dtype))
    ...
    kl_scale = optax.linear_schedule(
        init_value       = 0.0,
        end_value        = config.kl_annealing_factor / dataloaders['num_data'],
        transition_steps = int(
            config.kl_annealing_epochs * config.optim_ne
    ...
    def _find_params_by_node_name(params, node_name):
        filtered_params = jax.tree_util.tree_map(_get_key_finder(node_name), params, is_leaf=_is_leaf_fun)
        filtered_params = [x for x in jax.tree_util.tree_leaves(filtered_params) if x is not None]
        return filtered_params

    def step_trn(state, batch, config, scheduler, bnn_rng, kl_scale):
        ...
        def loss_fn(params):
            ...
            _, new_model_state = state.apply_fn({
                ...
                }, jnp.tile(batch['images'], (config.ensemble_size, 1, 1, 1)),
                rngs                = {'rank_one_bnn': bnn_rng},
            ...
            logits = logits.reshape(config.ensemble_size, -1, logits.shape[-1])         # [M, B, K,]
            target = jnp.expand_dims(common_utils.onehot(
                batch['labels'], num_classes=logits.shape[-1]), axis=0)                 # [1, B, K,]
            loss = -jnp.sum(target * jax.nn.log_softmax(logits, axis=-1), axis=-1)      # [M, B,   ]
            loss = jnp.mean(loss, axis=0)                                               # [   B,   ]
            loss = jnp.sum(
                jnp.where(batch['marker'], loss, jnp.zeros_like(loss))
            ) / jnp.sum(batch['marker'])

            # compute kl
            params_r_mean   = _find_params_by_node_name(params, 'rank_one_bnn_r_mean')
            params_r_rawstd = _find_params_by_node_name(params, 'rank_one_bnn_r_rawstd')
            params_r_stddev = [jax.nn.softplus(e) for e in params_r_rawstd]

            params_s_mean   = _find_params_by_node_name(params, 'rank_one_bnn_s_mean')
            params_s_rawstd = _find_params_by_node_name(params, 'rank_one_bnn_s_rawstd')
            params_s_stddev = [jax.nn.softplus(e) for e in params_s_rawstd]

            kl = sum([jnp.sum(
                    jnp.log(config.stddev / s)
                    + 0.5 * (s / config.stddev)**2
                    + 0.5 * ((0 - m) / config.stddev)**2
                    - 0.5) for m, s in zip(params_r_mean, params_r_stddev)
                ]) + sum([jnp.sum(
                    jnp.log(config.stddev / s)
                    + 0.5 * (s / config.stddev)**2
                    + 0.5 * ((0 - m) / config.stddev)**2
                    - 0.5) for m, s in zip(params_s_mean, params_s_stddev)
                ])

            # log metrics
            metrics = OrderedDict({'loss': loss, 'kl': kl})
            return loss + kl * kl_scale(state.step), (metrics, new_model_state)
        ...
        metrics['kl_scale'] = kl_scale(state.step)
    ...
    def step_val(state, batch):
        ...
        logits = new_model_state['intermediates']['cls.logit'][0]
        logits = logits.reshape(config.ensemble_size, -1, logits.shape[-1]) # [M, B, K,]
        predictions = jnp.mean(jax.nn.softmax(logits, axis=-1), axis=0)     # [   B, K,]
        acc = evaluate_acc(predictions, batch['labels'], log_input=False, reduction='none') # [B,]
        nll = evaluate_nll(predictions, batch['labels'], log_input=False, reduction='none') # [B,]
    ...
    p_step_trn         = jax.pmap(partial(step_trn, config=config, scheduler=scheduler, kl_scale=kl_scale), axis_name='batch')
...
def main():
    ...
    parser.add_argument('--ensemble_size', default=4, type=int,
                        help='the number of ensembles (default: 4)')
    parser.add_argument('--stddev', default=0.01, type=float,
                        help='prior standard deviation (default: 0.01)')
    parser.add_argument('--kl_annealing_factor', default=1.0, type=float,
                        help='it scales the kl term (default: 1.0)')
    parser.add_argument('--kl_annealing_epochs', default=0.75, type=int,
                        help='it specifies annealing epochs for the kl term (default: 0.75)')
